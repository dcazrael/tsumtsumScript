function TaskController() {
  (this.tasks = {}), (this.isRunning = !1), (this.interval = 200);
}
(TaskController.prototype.getFirstPriorityTaskName = function () {
  ...
  for (var s in this.tasks) {
    var i = this.tasks[s];
    n - i.lastRunTime < i.interval ||
      (null !== t
        ? i.priority < t.priority
          ? (t = i)
          : i.interval > t.interval
          ? (t = i)
          : i.lastRunTime < t.lastRunTime && (t = i)
        : (t = i));
  }
  return null === t ? '' : t.name;
}),
  (TaskController.prototype.loop = function () {
    for (console.log('loop start'); this.isRunning; ) {
      var t = this.getFirstPriorityTaskName(),
        n = this.tasks[t];
      void 0 !== n &&
        (n.run(),
        (n.lastRunTime = Date.now()),
        n.runTimes--,
        0 === n.runTimes && delete this.tasks[t]),
        sleep(this.interval);
    }
    (this.isRunning = !1), console.log('loop stop');
  }),
  (TaskController.prototype.updateRunInterval = function (t) {
    t < this.interval && t >= 50 && (this.interval = t);
  }),
  (TaskController.prototype.newTaskObject = function (t, n, s, i, o) {
    return {
      name: t,
      run: n,
      interval: s || 1e3,
  ...
  (TaskController.prototype.newTask = function (t, n, s, i, o) {
    if ((void 0 === o && (o = !1), 'function' == typeof n)) {
      var e = this.newTaskObject(t, n, s, i, 0);
      o && (e.lastRunTime = Date.now()), this.updateRunInterval(e.interval);
      var r = 'system_newTask_' + t,
        a = this.newTaskObject(
          r,
          function () {
            this.tasks[t] = e;
          }.bind(this),
      ...
      return (this.tasks[r] = a), e;
    }
    console.log('Error not a function', t, n);
  }),
  (TaskController.prototype.removeTask = function (t) {
    var n = 'system_removeTask_' + Date.now().toString(),
      s = this.newTaskObject(
        n,
        function () {
          delete this.tasks[t];
        }.bind(this),
  ...
  (TaskController.prototype.removeAllTasks = function () {
    var t = 'system_removeAllTask_' + Date.now().toString(),
      n = this.newTaskObject(
        t,
        function () {
          for (var t in this.tasks) delete this.tasks[t];
        }.bind(this),
        0,
        1,
        -20
  ...
  (TaskController.prototype.stop = function () {
    this.isRunning && ((this.isRunning = !1), console.log('wait loop stop...'));
  });
    ...
    if (gTaskController != undefined && gTaskController.tasks != undefined) {
      ...
      if (sendTask != undefined) {
        if (sendTask.lastRunTime == 0) {
          ...
          var next =
            (nowTime() - (sendTask.lastRunTime + sendTask.interval)) / 60000;
          msg += '/' + +next.toFixed(0);
        }
      }
    }
...
var Button = {
  ...
  gameSkillOn: {
    x: 160,
    y: 1490 + adjY,
    color: { 'a': 0, 'b': 0, 'g': 220, 'r': 238 },
  },
  gameSkillOff1: {
    x: 160,
    y: 1630 + adjY,
    color: { 'a': 0, 'b': 157, 'g': 112, 'r': 85 },
  },
  gameSkillOff2: {
    x: 160,
    y: 1630 + adjY,
    color: { 'a': 0, 'b': 181, 'g': 139, 'r': 72 },
  },
  gameSkillOff3: {
    x: 160,
    y: 1630 + adjY,
    color: { 'a': 0, 'b': 128, 'g': 73, 'r': 16 },
  },
  gameSkillOff4: {
    x: 160,
    y: 1630 + adjY,
    color: { 'a': 0, 'b': 178, 'g': 153, 'r': 3 },
  },
  gameRand: {
    x: 985,
    y: 1580 + adjY,
    color: { 'a': 0, 'b': 6, 'g': 180, 'r': 232 },
  },
  gamePause: {
    x: 983,
    y: 250 + adjY,
    color: { 'a': 0, 'b': 9, 'g': 188, 'r': 239 },
  },
  gameContinue: {
    x: 540,
    y: 1270 + adjY,
    color: { 'a': 0, 'b': 13, 'g': 175, 'r': 240 },
  },
  gameContinue1: {
    x: 461,
    y: 980 + adjY,
  ...
  outGameEnd: {
    x: 890,
    y: 1520 + adjY,
    color: { 'a': 0, 'b': 15, 'g': 140, 'r': 245 },
  },
  outStart1: {
    x: 500,
    y: 1520 + adjY,
    color: { 'a': 0, 'b': 19, 'g': 145, 'r': 247 },
  }, // 開始遊戲
  outStart2: {
    x: 500,
    y: 1520 + adjY,
    color: { 'a': 0, 'b': 129, 'g': 111, 'r': 236 },
  }, // 開始
  outClose: {
    x: 500,
    y: 1520 + adjY,
    color: { 'a': 0, 'b': 7, 'g': 180, 'r': 236 },
  }, // 關閉
  ...
  outReceiveOk: {
    x: 835,
    y: 1020 + adjY,
    color: { 'a': 0, 'b': 6, 'g': 175, 'r': 236 },
  },
  outReceiveClose: { x: 530, y: 1300 + adjY },
  outReceiveOne: {
    x: 840,
    y: 497 + adjY,
    color: { 'a': 0, 'b': 30, 'g': 181, 'r': 235 },
    color2: { 'a': 0, 'b': 119, 'g': 74, 'r': 40 },
  },
  outReceiveOne2th: {
    x: 840,
    y: 774,
  ...
  outReceiveOneHeart: {
    x: 290,
    y: 585 + adjY,
    color: { 'a': 0, 'b': 146, 'g': 65, 'r': 214 },
  },
  outReceiveOneCoin: {
    x: 291,
    y: 579 + adjY,
    color: { r: 232, g: 229, b: 38 },
  },
  outReceiveOneRuby: {
    x: 295,
    y: 579 + adjY,
    color: { r: 224, g: 93, b: 101 },
  }, // ruby
  outReceiveOneRuby2th: {
    x: 295,
    y: 651 + 68 * 3,
    color: { r: 235, g: 93, b: 105 },
  }, // ruby
  outReceiveOneAd: { x: 290, y: 812 - 140, color: { r: 90, g: 57, b: 25 } }, // ad
  outReceiveOneAd2th: {
    x: 290,
    y: 672 + 68 * 3,
    color: { r: 90, g: 57, b: 25 },
  }, // ad
  outReceiveOneTicket1: {
    x: 298,
    y: 569 + adjY,
    color: { r: 125, g: 188, b: 177 },
  }, // green
  outReceiveOneTicket2: {
    x: 316,
    y: 576 + adjY,
    color: { r: 248, g: 255, b: 253 },
  }, // white
  outIsLoading1: {
    x: 540,
    y: 720 + adjY,
  ...
  outSendHeart0: {
    x: 910,
    y: 626 + adjY,
    color: { 'a': 0, 'b': 142, 'g': 60, 'r': 209 },
    color2: { 'a': 0, 'b': 140, 'g': 65, 'r': 3 },
  },
  outSendHeart1: {
    x: 910,
    y: 823 + adjY,
  ...
  outSendHeartEnd: {
    x: 328,
    y: 1194 + adjY,
    color: { 'a': 0, 'b': 132, 'g': 85, 'r': 47 },
  },
  outSendHeartEnd2: {
    x: 227,
    y: 1190 + adjY,
    color: { 'a': 0, 'b': 123, 'g': 78, 'r': 44 },
  },
  outSendHeartEnd3: { x: 316, y: 1152 + adjY, color: { r: 55, g: 91, b: 139 } },
  outFriendScoreFrom: {
    x: 550,
    y: 863 + adjY,
    color: { 'a': 0, 'b': 140, 'g': 93, 'r': 55 },
  },
...
function printMaxScores(tsumMaxScores) {
  ...
  for (var i = 0; i < 10 && i < tsumMaxScores.length; i++) {
    str +=
      i + ', ' + tsumMaxScores[i].key + ', ' + tsumMaxScores[i].score + '    ';
  }
  log(str);
}
...
Tsum.prototype.init = function (detect) {
  ...
  if (this.screenHeight / this.screenWidth < 1.5) {
    ...
    this.gameOffsetY = Math.floor(
      ((this.gameWidth * 16) / 9 - this.gameHeight) / 2
    );
...
Tsum.prototype.sendMoneyInfo = function () {
  ...
  var x = Math.floor(
    Button.moneyInfoBox.x * this.captureGameRatio - this.gameOffsetX
  );
  var y = Math.floor(
    Button.moneyInfoBox.y * this.captureGameRatio - this.gameOffsetY
  );
  ...
  var img = getScreenshotModify(
    ...
    w,
    h,
    Button.moneyInfoBox.w / 2,
    Button.moneyInfoBox.h / 2,
    80
  );
...
Tsum.prototype.toResizeXY = function (x, y) {
  var rx = Math.floor(
    (x * this.captureGameRatio - this.gameOffsetX) / this.resizeRatio
  );
  var ry = Math.floor(
    (y * this.captureGameRatio - this.gameOffsetY) / this.resizeRatio
  );
  return { x: rx, y: ry };
};
...
Tsum.prototype.linkTsums = function (path) {
  for (var j in path) {
    ...
    var x = Math.floor(
      this.playOffsetX +
        ((point.x + Config.tsumWidth / 2) * this.playWidth) /
          this.playResizeWidth
    );
    var y = Math.floor(
      this.playOffsetY +
        ((point.y + Config.tsumWidth / 2) * this.playHeight) /
          this.playResizeHeight
    );
    if (j == 0) {
      tapDown(x, y, 10);
    ...
    for (var t = 0; t < times; t++) {
      ...
      for (var key in Page) {
        ...
        for (var i = 0; i < page.colors.length; i++) {
          var diff = absColor(
            page.colors[i],
            this.getColor(img, page.colors[i])
          );
...
Tsum.prototype.clearAllBubbles = function (startDelay, endDelay, fromY) {
  ...
  for (
    var bx = Button.gameBubblesFrom.x;
    bx <= Button.gameBubblesTo.x;
    bx += 140
  ) {
...
Tsum.prototype.useSkill = function (board) {
  ...
  for (var i = 0; i < 2; i++) {
    var img = this.screenshot();
    var isSkillOff1 = isSameColor(
      Button.gameSkillOff1.color,
      this.getColor(img, Button.gameSkillOff1),
      60
    );
    var isSkillOff2 = isSameColor(
      Button.gameSkillOff2.color,
      this.getColor(img, Button.gameSkillOff2),
      60
    );
    var isSkillOff3 = isSameColor(
      Button.gameSkillOff3.color,
      this.getColor(img, Button.gameSkillOff3),
      60
    );
    var isSkillOff4 = isSameColor(
      Button.gameSkillOff4.color,
      this.getColor(img, Button.gameSkillOff4),
      60
    );
  ...
  if (this.skillType == 'block_lukej_s') {
    ...
    for (var i = 0; i < 3; i++) {
      for (
        var bx = Button.gameBubblesFrom.x - 40;
        bx <= Button.gameBubblesTo.x + 40;
        bx += 150
      ) {
        for (
          var by = Button.gameBubblesFrom.y;
          by <= Button.gameBubblesTo.y + 100;
          by += 150
        ) {
          this.tap({ x: bx, y: by }, 10);
        }
    ...
    this.skillType == 'block_marie_s' ||
    this.skillType == 'block_missbunny_s' ||
    this.skillType == 'block_rabbit_s'
  ) {
    ...
    this.clearAllBubbles(
      10,
      50,
      (Button.gameBubblesFrom.y + Button.gameBubblesTo.y) / 2
    );
...
Tsum.prototype.scanBoardQuick = function () {
  ...
  if (this.debug) {
    saveImage(
      srcImg,
      this.storagePath + '/tmp/boardImg-' + this.runTimes + '.jpg'
    );
  }
  releaseImage(srcImg);
...
Tsum.prototype.recognizeSender = function (img) {
  ...
  var nameImg = cropImage(
    img,
    Math.floor(from.x),
    Math.floor(from.y),
    Math.floor(to.x - from.x),
    Math.floor(to.y - from.y)
  );
  var score = 0;
  var existFilename = '';
...
Tsum.prototype.countReceiveHeart = function (existFilename) {
  ...
  log(
    this.logs.receiveHeartFromHeartSender,
    this.record[existFilename].receiveCounts[dayTime],
    this.logs.hearts
  );
};
...
Tsum.prototype.isLoading = function () {
  ...
  for (var y = fy; y <= ty; y += 10) {
    if (
      isSameColor(
        Button.outIsLoading1.color,
        this.getColor(img, { x: fx, y: y }),
        20
      )
    ) {
      whiteCount++;
...
Tsum.prototype.taskReceiveOneItem = function () {
  ...
  while (this.isRunning) {
    var img = this.screenshot();
    var isItem = isSameColor(
      Button.outReceiveOne.color,
      this.getColor(img, Button.outReceiveOne),
      35
    );
    var isRuby = isSameColor(
      Button.outReceiveOneRuby.color,
      this.getColor(img, Button.outReceiveOneRuby),
      35
    );
    var isNonItem = isSameColor(
      Button.outReceiveOne.color2,
      this.getColor(img, Button.outReceiveOne),
      35
    );
    var isAd = isSameColor(
      Button.outReceiveOneAd.color,
      this.getColor(img, Button.outReceiveOneAd),
      35
    );
    var isOk = isSameColor(
      Button.outReceiveOk.color,
      this.getColor(img, Button.outReceiveOk),
      35
    );
    var isTimeout = isSameColor(
      Button.outReceiveTimeout.color,
      this.getColor(img, Button.outReceiveTimeout),
      35
    );
    releaseImage(img);
    if (isItem) {
...
Tsum.prototype.friendPageGoTop = function () {
  this.tapDown(
    { x: Button.outSendHeart3.x - 10, y: Button.outSendHeart0.y },
    100
  );
  this.moveTo(
    { x: Button.outSendHeart3.x - 10, y: Button.outSendHeart0.y },
...
Tsum.prototype.taskSendHearts = function () {
  ...
  while (this.isRunning) {
    ...
    for (var y = hfy; y <= hty; y += 8) {
      var isHs = isSameColor(
        Button.outSendHeart0.color,
        this.getColor(img, { x: hfx, y: y }),
        40
      );
      if (isHs) {
        heartsPos.push({
          x: hfx,
          y: y,
          color: Button.outSendHeart0.color,
          color2: Button.outSendHeart0.color2,
        });
        y += 140;
      }
    }
    ...
    var sy =
      heartsPos.length == 0 ? Button.outFriendScoreFrom.y : heartsPos[0].y + 35;
    for (var px = fx; px <= tx; px += 20) {
      isZero = isSameColor(
        Button.outFriendScoreFrom.color,
        this.getColor(img, { x: px, y: sy }),
        40
      );
      if (!isZero) {
        break;
    ...
    var isNotEnd = isSameColor(
      Button.outSendHeartEnd2.color,
      this.getColor(img, Button.outSendHeartEnd2),
      40
    ); //x: 75, y: 420
    var isEnd1 = isSameColor(
      { r: 162, g: 84, b: 53 },
      this.getColor(img, { x: 75 * 3, y: 420 * 3 }),
      40
    ); // {x: 75, y: 420, r: 162, g: 84, b: 53}}
    var isEnd2 = isSameColor(
      Button.outSendHeartEnd.color,
      this.getColor(img, Button.outSendHeartEnd),
      40
    ); // x: 109, y: 422
    var isEnd3 = isSameColor(
      Button.outSendHeartEnd3.color,
      this.getColor(img, Button.outSendHeartEnd3),
      40
    ); // x: 105, y: 408

    var isNotEndJP = isSameColor(
      Button.outSendHeartEnd2.color,
      this.getColor(img, { x: 75 * 3, y: 352 * 3 }),
      40
    ); //x: 75, y: 352
    var isEndJP1 = isSameColor(
      { r: 162, g: 84, b: 53 },
      this.getColor(img, { x: 75 * 3, y: 352 * 3 }),
      40
    ); // {x: 75, y: 352, r: 162, g: 84, b: 53}}
    var isEndJP3 = isSameColor(
      Button.outSendHeartEnd3.color,
      this.getColor(img, { x: 105 * 3, y: 340 * 3 }),
      40
    ); // x: 105, y: 340
    ...
    if (
      (heartsPos.length == 0 && (isEnd || isEndJP)) ||
      (!this.sentToZero && isZero && heartsPos.length != 0)
    ) {
      if (retry < 3) {
        this.tapDown(
          { x: Button.outSendHeart3.x - 10, y: Button.outSendHeart3.y },
          50
        );
        this.moveTo(
          { x: Button.outSendHeart3.x - 10, y: Button.outSendHeart3.y },
        ...
        this.moveTo(
          { x: Button.outSendHeart3.x - 10, y: Button.outSendHeart2.y },
          50
        );
        this.moveTo(
          { x: Button.outSendHeart3.x - 10, y: Button.outSendHeart1.y },
          50
        );
        this.moveTo(
          { x: Button.outSendHeart3.x - 10, y: Button.outSendHeart0.y },
        ...
        this.moveTo(
          { x: Button.outSendHeart3.x - 10, y: Button.outSendHeartTop.y },
          500
        );
        this.tapUp(
          { x: Button.outSendHeart3.x - 10, y: Button.outSendHeartTop.y },
...
function start(
  ...
  if (receiveOneItem) {
    gTaskController.newTask(
      'receiveOneItem',
      ts.taskReceiveOneItem.bind(ts),
      receiveOneItemInterval * 60 * 1000,
      0
    );
  }
  if (receiveItem) {
    gTaskController.newTask(
      'receiveItems',
      ts.taskReceiveAllItems.bind(ts),
      receiveItemInterval * 60 * 1000,
      0
    );
  }
  if (sendHearts) {
    gTaskController.newTask(
      'sendHearts',
      ts.taskSendHearts.bind(ts),
      sendHeartsInterval * 60 * 1000,
      0
    );
  }
...
function genRecordTable() {
  ...
  html += '<table>';
  html +=
    '<tr><td>UserImage</td><td>UserImage2</td><td>All</td><td>Avg</td><td>Day</td></tr>';
  var dayMapCount = {};
